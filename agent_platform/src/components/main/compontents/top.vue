<template>
	<div class="infor-affix clearfix" v-if="userInfro && userInfro.uc_id && toggleFlag">
...
export default {
	...
	methods: {
		...
		async _getSysUserInfo() {
			...
			this.userInfro = (storeUser.userInfo && difTimeFlag) ? storeUser.userInfo : await this.getSysUserInfo()
		...
		setTipsText() {
			switch (this.userInfro.status) {
				...
				case 4:
					...
					break
				case 5:
					this.tipsText = `账户信息审核未通过，请按照提示原因修改账户信息。原因:${this.userInfro.note}`
					break
			}
		},
		/**
