<template>
	<div class="upload-image" :style="{width: widthVal+'px', height: heightVal+'px'}">
		...
		<div class="logo-image" :style="{width: widthVal-8+'px', height: heightVal-8+'px',lineHeight:heightVal-8 + 'px'}" v-show="value || loading">
			<img :src="value" v-if="value && formDataType === 2" class="ry-align-center" :style="{maxWidth: widthVal-10+'px', maxHeight: heightVal-10 +'px'}">

			<video :src="value" v-if="value && formDataType === 3" controls="controls" class="ry-align-center" :style="{maxWidth: widthVal-10+'px', maxHeight: heightVal-10 +'px'}">
					您的浏览器不支持 video 标签。
			</video>

			<div class="cover" :style="{width: widthVal-8+'px', height: heightVal-8+'px',lineHeight:heightVal-8+'px' }" :class="{'is-edit': disEdit}">
				<div class="cover_content" :style="{width: widthVal-8+'px', height: heightVal-8+'px'}">
					<div class="left">
	...
	methods: {
		...
		checkMaxBit(bit) {
			if ((bit / 1000) > this.bit) {
				this.$Message.error({content: '超过文件大小限制', duration: 3})
				return false
			}
		...
		checkMinWidth(width) {
			if (this.minWidth && width < this.minWidth) {
				this.$Message.error({content: '图片小于规定尺寸', duration: 3})
				return false
			}
		...
		checkMaxWidth(width) {
			if (this.maxWidth && width > this.maxWidth) {
				this.$Message.error({content: '图片大于规定尺寸', duration: 3})
				return false
			}
		...
		checkW2HRatio(width, height) {
			if (!this.isCalcErrorRatio && !!this.ratio) {
				...
				if (width / height !== _w / _h) {
					this.$Message.error({content: '图片尺寸不符合规格', duration: 3})
					return false
				}
			}
