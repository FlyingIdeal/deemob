<template>
  <div class="page-main">
    <div class="page-title page-has-line">
       <h2 class="title_hover" @click="toLink">账户审核日志</h2>
       <h2 style="margin-left: 18px" >创意审核日志</h2>
    </div>
...
<script>
  ...
  import {getCreativeRecords, getAdvertiserList} from '@/api/audit'
  import {getCategoryList} from '@/api/common'
  import {getAuditReCols} from '../../common/colList'
  ...
  export default {
    ...
    data() {
      return {
        ...
        dateCreateTime: [], // 创建时间
        datePickerTime: [], // 投放时间
        tableColList: getAuditReCols(this),
    ...
    created() {
      if (this._hasPageAccess({targetRoute: 'audit-log-account'})) {
        this.initData()
      }

      this._inPageAccessArr({targetRoute: 'audit-log-account'}) // 获取页面按钮权限数组
    },
    methods: {
      /**
       * [initData 初始化加载数据]
       ...
       */
      toDetail(row, type) {
        return `/audit/log/creative/detail?creative_info_id=${row.id}&type=${type}&form=log`
      },

      toLink() {
        this.$router.push({name: 'audit-log-account'})
      },
      /**
       * [getTimeData 获取行业列表]
       * @return {[type]} [description]
      ...
      getTableList() {
        ...
        this.filterSearch.industry_id ? obj.category_sub_id = this.filterSearch.industry_id : ''
        ...
        if (this.dateCreateTime[0] && this.dateCreateTime.length > 1) {
          obj.created_since = this.dateCreateTime[0]
          obj.created_until = this.dateCreateTime[1]
        }
