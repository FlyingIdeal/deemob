...
<!--              <Badge v-if="row.status === 1" color="#F1A32E" text="待审核"/>-->
              ...
              <Badge v-if="row.status === 5" color="#F93D3D" text="已拒绝"/>
            </template>
            <template slot-scope="{ row }" slot="status_desc">
              <p class="status_des" :title="row.note" v-if="row.status === 5 && row.note"> {{ row.note }}</p>
              <p v-else> - </p>
            </template>
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       */
      toDetail(row, type) {
        return `/audit/log/account/detail?id=${row.id}&type=${type}&account_type=${row.account_type}&form=log`
      },
      toLink() {
