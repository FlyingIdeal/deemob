  ...
  <div class="page-main">
    <div class="page-title page-has-line clearfix">
      ...
      <div class="is-handler">
        <Button v-if="_isShow_('addEditDel')" style="float: right" icon="ios-add" type="primary" @click="toggleFormDraw()">新建账号</Button>
      </div>
    </div>
    <Card :bordered="false" dis-hover class="has-table-card">
      <div class="search-line">
        <Row>
          <Col span="24">
            <Input :clearable="true"  class="i-margin-right-11 i-width-input mar-bot-10" v-model.trim="search.name" placeholder="输入账户名称或姓名"/>
     ...
     <div class="console-table">
       <Table
         ...
         <template slot-scope="{ row }" slot="user_name">
           <p class="name-row-ellipsis" :title="row.user_name">{{ row.user_name }}</p>
         </template>
         <template slot-scope="{ row }" slot="status">
           <span v-if="row.status === 1">正常</span>
           <span v-if="row.status === -1">停用</span>
           <i-switch v-if="_isShow_('addEditDel')" size="small" :true-value="1" :false-value="-1" v-model="row.status" @on-change="changeState(row)"/>
         </template>
          <!--关联代理商-->
         ...
         <template slot-scope="{ row, index }" slot="action">
           <Button v-if="_isShow_('addEditDel')" type="text" @click="doCreate(row)">修改</Button>
    ...
    <Drawer v-model="setModalFlag"  class="custom-drawer" width="600px" :mask-closable="false" :closable="false" :styles="drawStyles">
      ...
      <Form ref="modalForm" label-position="right" :model="modalForm" :rules="modalFormRules" :label-width="100">
        ...
        <FormItem label="角色" prop="role_ids">
          <Select :transfer="true" filterable v-model="modalForm.role_ids" placeholder="请选择角色">
        ...
        <FormItem  prop="all_agent">
          <label slot='label'>
            ...
            <Tooltip placement="top" max-width="235">
              ...
              <div slot="content">指该账号可操作的代理商账号，同时影响报表对应的资源数据查看</div>
            </Tooltip>
          </label>
