...
export function validateCreditCode(rule, value, callback) {
	...
	if (value === '' || value === undefined || value === null) {
		...
		if ((!reg.test(value)) && value !== '') {
			callback(new Error('请输入正确的社会统一信用编码'))
		} else {
			callback()
		}
	}
...
/**
 * [fileVideoReader2Promise 读取文件 转化为 promise]
 * @param  {[type]} file [description]
 * @return {[type]}      [description]
 */
export function fileVideoReader2Promise(file) {

	let typeArr = ['quicktime'] //兼容处理，由格式工场转化输出的type类型为quicktime，取name的值

	let _type = (!!file.type && file.type.indexOf('/') > -1) ? file.type.split('/')[1].toLowerCase() : null

	if (typeArr.includes(_type) || !_type) {
		let nameArr = file.name.split('.')
		_type = nameArr[nameArr.length - 1].toLowerCase()
	}
