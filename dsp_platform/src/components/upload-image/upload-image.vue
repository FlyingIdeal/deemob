<template>
	<div class="upload-center">
		<div class="upload-middle">
			<div class="upload-item" v-for="(item, index) in dataImgArr">
				<div class="upload-image">
					<div class="logo-image" :style="fixAreaStyle">
						<img :src="item.url" v-if="item.loading && item.url" class="ry-align-center" :style="maxAreaStyle">
			...
			<div class="upload-item" v-show="isAllLoading && (multiple || dataImgArr.length === 0 || dataImgArr.length < multipleLen) && !this.disEdit">
				<!-- 上传插件显示条件 1.多图； 2.dataImgArr.length === 0; 3. 可编辑 -->
				<div class="upload-image">
					<Upload
						:multiple="multiple"
						:action="actionUrl"
...
import { fileReader2Promise, debounce, getQueryString } from '@/libs/tools'
import { dataType, imageFormat } from '@/libs/util'
...
export default {
	...
	data() {
		return {
			actionUrl: uploadFileUrl, // action的url
			isAllLoading: true, // 已全部上传完成, 默认事
			noConformArr: [], // 不符合文件规格的文件,最后提示使用
			reUploadIndex: -1, // 点击重新上传的index
	...
	methods: {
		...
		resetUpload(index) {
			...
			this.reUploadIndex = index // 重新上传的index
			...
			this.dataImgArr.map(item => {
				if(!!item.url) {
					if(!!this.showUploadName) {
						...
						// 上传到素材，需要宽+高+大小kb+时长
						_arr.push(`${item.url}?materialId=${item.materialId}&name=${escape(item.name)}&type=${info.type}&width=${info.width}&height=${info.height}&bit=${info.bit}&duration=${info.duration}`)
					} else {
						_arr.push(item.url)
		...
		toDoUpload(conformFileList){
			try {
				...
				if (_this.reUploadIndex >= 0) { // 点击重新上传

					// 点击再次上传只能单张上传，so
					conformFileList = [conformFileList[0]]

					_this.dataImgArr.splice(_this.reUploadIndex, 1, conformFileList[0])
				} else {
				...
				for(let i = 0; i < conformFileList.length; i++) {
					(function(index) {
						...
						let _index = _this.reUploadIndex >= 0 ? _this.reUploadIndex : (preDataImgArrLen + index)  // 兼容再次上传
						...
						uploadFile(formData).then(res => {
							return res.code === 200 ? `${res.data.host}${res.data.url[0]}` : false
						}, err => {
							return false
						}).then((value) => {

							_this.$set(_this.dataImgArr[_index], 'url', !!value ? value : '')
				...
				if (!!_resultArr.length) {
					...
					_resultArr.map(item => {
						...
						if (!!item) {
							...
							_arr.push({
								loading: true,
								materialId: getQueryString('materialId', item), // 素材id
								name: getQueryString('name', item),
