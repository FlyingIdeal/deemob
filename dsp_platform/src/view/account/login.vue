<template>
	<div class="login clearfix">
		<div class="login-left fl">
			<div class="img-card">
				<img src="~@/assets/image/login/login-left-img.png" alt="">
			</div>
		</div>
		<div class="login-right fr">
			<div class="login-card">
				...
				<div class="plat-hi">Hi，欢迎进入广告投放平台~</div>
...
import { verifySession, getCaptchaCode } from '@/api/user'
...
export default {
	...
	created() {
		...
		if(this.tag === 'pass' || !localStorage.getItem('userId')) {
			this.resetInit()
		} else {
	...
	methods: {
		...
		initMain() {
			...
			verifySession({platform_id: this.$config.platformId}).then(res => {

				this.$Spin.hide()

				if (res.code === 200 && !!res.data.uc_id) {
