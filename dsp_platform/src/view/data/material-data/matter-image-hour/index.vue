  ...
  <div class="page-main">
    ...
    <div class="search-line" ref="search-line" style="margin-top:10px">
      ...
      <Select
        ...
        <Option v-for="item in groupSearchList" :value="item.id" :key="'小时搜索组'+item.id">{{ item.material_group_name }}({{ item.id }})</Option>
      </Select>
      ...
      <Select
        ...
        <Option v-for="item in tagSearchList" :value="item.id" :key="'小时搜索标签'+item.id">{{ item.material_tag_name }}({{ item.id }})</Option>
      </Select>
    ...
    <div class="data-table console-table">
      <Table :loading="tableLoadFlag" border stripe :columns="tableColList" :data="matterDayData"
        ...
        <template slot-scope="{ row }" slot="hour">
          <p> {{ row.timestamp | timeFormat('hh:mm:ss') }}</p>
        </template>
        <!-- 素材图-->
          ...
          <Poptip trigger="hover" placement="right" transfer>
            ...
            <div class="detail-material-zoomup" slot="content">
              <div class="zoomup-inner">
                <img :src="row.material_link" v-if="row.material_type === 1">
                <video :src="row.material_link" controls="controls" v-if="row.material_type === 2">
                  您的浏览器不支持 video 标签。
                </video>
              </div>
            </div>
...
<script>
  ...
  import {matterHourColumns} from '../colList'
  ...
  export default {
    ...
    data() {
      return {
        ...
        tableColList: matterHourColumns(this),
    ...
    methods: {
      ...
      getAxiosParams() {
        ...
        let params = {
          data_type: 'hour',
          material_type: 1, // 素材类型，1=图片，2=视频
