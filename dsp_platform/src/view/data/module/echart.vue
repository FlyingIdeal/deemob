		...
		<div class="monitor-card" v-show="moniterData.date && moniterData.date.length">
			<div class="diff-card" v-if="selectArr.length">
				<Select class="pre-select" v-model="preValue" placeholder="筛选" @on-change="resetMoniter">
			        <Option :disabled="item.key === nextValue" v-for="item in selectArr" :value="item.key" :key="item.key">{{item.title}}</Option>
			    </Select>
			    ...
			    <Select class="next-select" v-model="nextValue" v-if="isContrast" placeholder="筛选" @on-change="resetMoniter">
			        <Option :disabled="item.key === preValue" v-for="item in selectArr" :value="item.key" :key="item.key">{{item.title}}</Option>
			    </Select>
			</div>
...
export default {
 	...
 	data() {
 		return {
 			colorList: ['#0AB9DE', '#8743F8'], // echart颜色列表
 			preValue: '', // 对比前的值
 			nextValue: '', // 对比后的值
