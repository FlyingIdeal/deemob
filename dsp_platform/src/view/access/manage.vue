        ...
        <Card :bordered="false" dis-hover>
            <div class="page-title clearfix">
                ...
                <div class="is-handler">
                    <Button v-if="_isShow_('addNewBudget')" icon="ios-add" type="primary" @click="doCreate()">新建版本</Button>
                </div>
            </div>
            <div class="console-table">
                <Table highlight-row stripe border :columns="accessColumns" :data="accessDataList" :loading="tableLoadFlag" :height="tableHeight" :no-data-text="noDataText">
                    ...
                    <template slot-scope="{ row }" slot="access_type">
                        ...
                        <span v-if="row.access_type === 3">Android-SDK</span>
                    ...
                    <template slot-scope="{ row }" slot="status">
                        ...
                        <i-switch v-if="(row.status === 1 || row.status === 2) && _isShow_('publish')" size="small" :true-value="1" :false-value="2" v-model="row.status" @on-change="changeState(row)"/>
