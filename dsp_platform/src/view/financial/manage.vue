    ...
    <div class="page-main">
        ...
        <Card :bordered="false" dis-hover class="has-table-card">
            <div class="search-line" ref="search-line">
                <DatePicker class="i-margin-right-11 i-width-date mar-bot-10" :value="datePickerTime" :transfer="true" :options="datePickerDisabled" :clearable="false" format="yyyy-MM-dd" type="daterange" placeholder="请选择日期" @on-change="datePickerChange"></DatePicker>
                <Select
            ...
            <div class="console-table">
                <Table highlight-row stripe border :columns="tableColList" :data="colDatList" :loading="tableLoadFlag" :no-data-text="noDataText">
                    ...
                    <template slot-scope="{ row }" slot="amount">
                        <template v-if="row.detail_type === 1">+</template>
                        {{ row.amount | number2Thousand }}
                    </template>
                    <!--明细-->
                    <template slot-scope="{ row }" slot="remark">
                        <!-- 1充值 2退款 3花费 -->
                        <template v-if="row.detail_type === 1">充值</template>
                        <template v-if="row.detail_type === 2">退款</template>
                        <template v-if="row.detail_type === 3">花费</template>
                    </template>
                </Table>
...
export default {
    ...
    methods: {
        ...
        getTableList() {
            ...
            //1充值 2退款 3花费
            if(!!this.filterSearch.detail_type) {
                params.detail_type = this.filterSearch.detail_type
            }
        ...
        downStatement() {
            ...
            let params = {
                method: 'GET',
                url: getFinanceListUrl({...this.filterSearch, start_date: this.datePickerTime[0], end_date: this.datePickerTime[1]})
            }
