<template>
  <Card :bordered="false" dis-hover class="page-main ad_config">
    ...
    <div class="config_con">
      <Form ref="deployData" label-position="left" :model="deployData1" :rules="deployRule" :label-width="145">
          ...
          <div class="config_header">
            ...
            <div>
              <div class="action" @click="changeState(1)">
                <p v-show="basicStatus === 1">收起<span> <Icon type="ios-arrow-down"/></span></p>
                <p v-show="basicStatus === 2">展开<span> <Icon type="ios-arrow-up"/></span></p>
              </div>
            </div>
          </div>
          <div style="margin-top: 15px" v-show="basicStatus === 1">
            <div class="config_matter">
              <FormItem class="deployText" label="广告位名称（ID）">
                <p>{{deployData['ssp_slot_name']}}（{{deployData['ssp_slot_id']}}）</p>
              </FormItem>
              <FormItem class="deployText" label="应用名称（ID）">
                <p>{{deployData['app_name']}}（{{deployData['app_id']}}）</p>
              </FormItem>
              <FormItem class="deployText" label="广告场景">
                <p>{{deployData['ad_type_text']}}-{{creativeId2Text(deployData['creative_type'])}}</p>
              </FormItem>
              ...
              <FormItem class="deployText deployForm" label="内部广告位名称">
                <Input v-model="deployData.ssp_slot_alias" style="width: 250px"/>
              </FormItem>
              <FormItem class="deployText deployForm" label="外部广告位ID">
                <p v-if="deployData['ex_slot_id']">{{deployData['ex_slot_id']}}</p>
                <p v-else>-</p>
              </FormItem>
              <FormItem v-if="deployData['access_type']==='SDK'"
                        v-for="(item,index) in deployData['platform_slot_code']"
                        :key="'platform_slot_code'+index" class="deployText deployForm"
            ...
            <FormItem class="deployText" prop="size_ratio">
               <span slot="label">
				...
				<CheckboxGroup  v-model="sizeArr" @on-change="sizeChange">
					<Checkbox v-for="(item,index) in adMaterialSizeArr" class="ivu-checkbox-size" :key="'素材'+item.scale+index"  :label="item.ratio">{{item.ratio}}</Checkbox>
				</CheckboxGroup>
<!--              <div class="size_card">-->
...
<!--              </div>-->
        ...
        <div class="config_detail">
          <div class="config_header">
            <div>
              <span class="line"></span><span>价格</span>
            </div>
            <div>
              <div class="action" @click="changeState(2)">
                <p v-show="priceStatus === 1">收起<span> <Icon type="ios-arrow-down"/></span></p>
                <p v-show="priceStatus === 2">展开<span> <Icon type="ios-arrow-up"/></span></p>
              </div>
            </div>
          </div>
          <div class="priceCon" style="margin-top: 15px" v-show="priceStatus===1">
          ...
          <span slot="label">
            ...
            <div slot="content">
               <p style="font-size: 12px">计费方式指该流量支持的广告投放方式（CPC,CPM），通过此方式和底价过滤出合适预算</p>
            </div>
         </Tooltip>
          </span>
              <div class="charge_content">
                <div class="charge_modal">
                  ...
                  <div class="charge_num">
                    <FormItem :label-width="85" class="deployForm" label="CPM底价">
                      <Poptip trigger="focus">
                        <InputNumber :min="0" :step="1" :max="inputMaxNumber" v-model="deployData.floor_price_cpm"
                                     style="width: 100px"/>  <span>元</span>
                        <div slot="content">{{ formatNumber(deployData.floor_price_cpm) }}</div>
                      </Poptip>
                    </FormItem>
                  </div>
                  <div class="charge_num" style="margin-left: 10px">
                    <FormItem :label-width="85" class="deployForm" label="CPC底价">
                      <Poptip trigger="focus">
                        <InputNumber :min="0" :step="1" :max="inputMaxNumber" v-model="deployData.floor_price_cpc"
                                     style="width: 100px"/>  <span>元</span>
                        <div slot="content">{{ formatNumber(deployData.floor_price_cpc) }}</div>
                      </Poptip>
                    </FormItem>
                  </div>
                </div>
            ...
            <div slot="content">
              ...
              <div class="settlement_content">
                <div class="settlement_check">
                  <RadioGroup v-model="deployData.settlement">
                    <Radio :label="1">
                      <span>保价</span>
                    </Radio>
                    <Radio :label="2">
                ...
                <div class="settlement_detail">
                  <div v-if="deployData.settlement===1">
                    <FormItem :label-width="85" prop="settlement" class="deployForm">
                      <span slot="label" style="display: inline-block">CPM价格</span>
                      <Poptip trigger="focus">
                        <InputNumber :min="0" :step="1" :max="inputMaxNumber" v-model="deployData.valuation_price"
                                     style="width: 100px"/>  <span>元</span>
                        <div slot="content">{{ formatNumber(deployData.valuation_price) }}</div>
                      </Poptip>
                    </FormItem>
                  </div>
                  <div v-if="deployData.settlement===2">
                    <FormItem :label-width="85" prop="settlement" class="deployForm">
                      <span slot="label">CPC价格</span>
                      <Poptip trigger="focus">
                        <InputNumber :min="0" :step="1" :max="inputMaxNumber" v-model="deployData.divide_price"
                                     style="width: 100px"/>  <span>元</span>
                        <div slot="content">{{ formatNumber(deployData.divide_price) }}</div>
                      </Poptip>
                    </FormItem>
                  </div>
                </div>
            ...
            <span slot="label">
             ...
             <Tooltip placement="top-start" :transfer="true" max-width="700">
               <Icon type="md-help-circle" style="color: #c8c8c8;font-size: 16px"/>
            <div slot="content">
               <p style="font-size: 12px">参与RTB竞价的出价，如不填默认是0无法竞得流量（例如与2345合作）</p>
            </div>
         </Tooltip>
          </span>
              <Poptip trigger="focus">
                <InputNumber :min="0" :step="1" :max="inputMaxNumber" v-model="deployData.cpm_price"
                             style="width: 100px"/>  <span>元</span>
                <div slot="content">{{ formatNumber(deployData.cpm_price) }}</div>
              </Poptip>
            </FormItem>
            <div class="priceCPC">
              <div class="priceCPC_con" style="background: linear-gradient(-59deg, #BD3B95, #FC6AA2);">
                <p class="priceDetail">实时媒体eCPM：<span class="priceNum">{{deployData.last_hour_ecpm}}</span>元</p>
                <p class="priceDetail">实时媒体CPC：<span class="priceNum">{{deployData.last_hour_cpc}}</span>元</p>
              </div>
              <div class="priceCPC_con" v-show="deployData.last_day_ecpm !==0"
                   style="background: linear-gradient(-36deg, #6051CB, #8A5FBB);">
                <p class="priceDetail">历史最新eCPM：<span class="priceNum">{{deployData.last_day_ecpm}}</span>元</p>
                <p class="priceDetail">历史最新CPC：<span class="priceNum">{{deployData.last_day_cpc}}</span>元</p>
              </div>
            </div>
          </div>
        </div>
        <div class="config_detail">
          <div class="config_header">
            <div>
              <span class="line"></span><span>扣量策略</span>
            </div>
            <div>
              <div class="action" @click="changeState(3)">
                <p v-show="strategyStatus === 1">收起<span> <Icon type="ios-arrow-down"/></span></p>
                <p v-show="strategyStatus === 2">展开<span> <Icon type="ios-arrow-up"/></span></p>
              </div>
            </div>
          </div>
          <div style="margin-top: 15px" v-show="strategyStatus === 1">
            <div class="config_matter">
              <FormItem class="deployForm deployText" label="广告展现保留比" prop="discount_show">
                <InputNumber :min="0" :max="100" v-model="deployData.discount_show" placeholder="大于0小于等于100的整数" style="width: 250px"/>
                <span style="margin-left: 5px">%</span>
              </FormItem>
              <FormItem class="deployForm deployText" label="广告点击保留比" prop="discount_click">
                <InputNumber :min="0" :max="100" v-model="deployData.discount_click" placeholder="大于0小于等于100的整数" style="width: 250px"/>
                <span>%</span>
              </FormItem>
            </div>
          </div>
        ...
        <div class="config_detail" style="margin-bottom: 10px">
          <div class="config_header">
            <div>
              <span class="line"></span><span>流量分配</span>
            </div>
            <div>
              <div class="action" @click="changeState(4)">
                <p v-show="flowStatus === 1">收起<span> <Icon type="ios-arrow-down"/></span></p>
                <p v-show="flowStatus === 2">展开<span> <Icon type="ios-arrow-up"/></span></p>
              </div>
            </div>
          </div>
        ...
        <FormItem class="deployForm configBottom" :label-width="0">
          ...
          <Button type="primary" :loading="submitSave" @click="save('deployData',1)">
    ...
    <selectBudget v-model="modalSelect" :sizeRatio="size_ratio" :dataList="deployData" :ids="parseInt(selected_id)"
                  :deleteArr="except_dsp_slot_id" @name="selectSuccess"/>
...
<script>
  ...
  import {getSspSlotConfigData, submitConfigApi, getFetchRealData, getDealList} from '@/api/ssp'
  ...
  export default {
    name: 'ad-config',
    components: {RyTimeSheet, configDelete, crowdControl, retargetMutiCascaderAdSlot, selectBudget},
    ...
    methods: {
      ...
      seeSuccess(row) {
        ...
        let size_ratio = row.width_ratio && row.height_ratio ? this.number2Thousand(row.width_ratio / row.height_ratio) : 0
      ...
      getConfigurationData() {
        ...
        getSspSlotConfigData({
          ...
          if (date.code === 200) {
            this.$Spin.hide()
            let list = date.data
            list.settlement = list.divide_price ? 2 : 1 // 判断是cpc价格还是cpm价格
            list.divide_price = list.divide_price ? Number(this.number2Thousand(list.divide_price / 100)) : 0 // cpc价格
            list.valuation_price = list.valuation_price ? Number(this.number2Thousand(list.valuation_price / 100)) : 0 // cpm价格
            list.cpm_price = list.cpm_price > 0 ? Number(this.number2Thousand(list.cpm_price / 100)) : 0 // cpm出价
            list.cpc = list.floor_price_cpc > -1 // 判断cpc是否选中
            list.floor_price_cpc = list.floor_price_cpc > 0 ? Number(this.number2Thousand(list.floor_price_cpc / 100)) : 0 // cpc价格
            list.cpm = list.floor_price_cpm > -1 // 判断cpm是否选中
            list.floor_price_cpm = list.floor_price_cpm > 0 ? Number(this.number2Thousand(list.floor_price_cpm / 100)) : 0 // cpm价格
            ...
            if (list.ad_ratio.length > 0) {
              this.size_ratio = (list.ad_ratio).map((item, index) => {
                item.scale = Number(this.number2Thousand(item.width / item.height))
      ...
      getResourceList(data) {
        ...
        let dataList = controllerList.map((item, q) => {
          ...
          let sizeRatio = item.width_ratio && item.height_ratio ? this.number2Thousand(item.width_ratio / item.height_ratio) : 0
          ...
          item.area_codes = item.area_type !== -1 ? item.area_codes.split(',') : []
          ...
          item.brand_ids = item.brand_type !== -1 ? item.brand_ids.split(',') : []
      ...
      errData(pass) {
        ...
        if (errOsList.length > 0 || errAdList.length > 0) {
          Object.keys(errOsList).forEach(h => {
            ...
            html += '<p style="font-size:14px;color:#333;line-height: 25px">当前广告位所配置的预算位：【' + errOsList[h].dsp_slot_name + '】的应用平台是【' + type + '】，与当前广告位应用平台【' + this.deployData['os_type'] + '】不符</p>'
          })
          Object.keys(errAdList).forEach(h => {
            html += '<p style="font-size:14px;color:#333;line-height: 25px">当前广告位所配置的预算位：【' + errAdList[h].dsp_slot_name + '】的广告场景是【' + errAdList[h].ad_type_text + '】，与当前广告位应用平台【' + this.deployData['ad_type_text'] + '】不符</p>'
          })
          html += '<p>确认提交么？</p>'
          this.$Modal.confirm({
            title: html,
      ...
      submitData(pass) {
        ...
        // cpm底价为0时传-1
        obj.floor_price_cpm = this.deployData.cpm ? parseInt(parseFloat(this.deployData.floor_price_cpm) * 100) : -1
        // cpc底价为0时传-1
        obj.floor_price_cpc = this.deployData.cpc ? parseInt(parseFloat(this.deployData.floor_price_cpc) * 100) : -1
        // cpm价格
        obj.valuation_price = this.deployData.settlement === 1 ? parseInt(parseFloat(this.deployData.valuation_price) * 100) : 0
        // cpc价格
        obj.divide_price = this.deployData.settlement === 1 ? 0 : parseInt(parseFloat(this.deployData.divide_price) * 100)
        // 启用到自营
    ...
    created() {
      ...
      if (this._isSuperAdmin() || !!resultArr.includes('setting') || !!resultArr.includes('passAndSet')) {
        ...
        this.basicStatus = parseInt(localStorage.getItem('basicStatus')) ? parseInt(localStorage.getItem('basicStatus')) : 1
        this.priceStatus = parseInt(localStorage.getItem('priceStatus')) ? parseInt(localStorage.getItem('priceStatus')) : 1
        this.strategyStatus = parseInt(localStorage.getItem('strategyStatus')) ? parseInt(localStorage.getItem('strategyStatus')) : 1
        this.flowStatus = parseInt(localStorage.getItem('flowStatus')) ? parseInt(localStorage.getItem('flowStatus')) : 1
        this.otherStatus = parseInt(localStorage.getItem('otherStatus')) ? parseInt(localStorage.getItem('otherStatus')) : 1
      } else {
        this._gotoNoAccess()
      }
    }
